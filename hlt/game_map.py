...
class Map:
    ...
    def get_player_previous(self, player_id):
        """
        :param int player_id: The id of the desired player
        :return: The player associated with player_id
        :rtype: Player previous position
        """
        return self._previous_players.get(player_id)
    ...
    def _parse(self, map_string):
        ...
        self._previous_players = self._players
    ...
    def no_planets_between(self, source, target, fudge=0.5 + 0.1):
        ...
        for planet in self.all_planets():
            if collision.intersect_segment_circle(source, target, planet, fudge=fudge):
                return False
        return True


    def obstacles_between(self, ship, target, ignore=(), fudge=0.5 + 0.1):
        """
        Check whether there is a straight-line path to the given point, without planetary obstacles in between.
        ...
        for foreign_entity in entities:
            if foreign_entity == ship or foreign_entity == target:
                continue
            if collision.intersect_segment_circle(ship, target, foreign_entity, fudge=ship.radius + fudge):
